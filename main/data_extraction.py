...
class DataExtractor(DatabaseConnector, DataCleaning):
    ...
    def retrieve_stores_data(self, store_details_endpoint, headers, num_stores):
        ...
        for store_index in range(1, num_stores + 1):
            url = store_details_endpoint.format(store_number=store_index)
            response = requests.get(url, headers=headers)
            if response.status_code == 200:
                self.all_stores_data.append(response.json())  # Assuming response is a JSON object with store details
            else:
                print(f"Failed to retrieve data for store number {store_index}. Status code: {response.status_code}")

        # Convert the list of store data to a DataFrame
        return pd.DataFrame(self.all_stores_data)    
...
if __name__ == "__main__":
    ...
    stores_df = de.retrieve_stores_data(store_details_endpoint, headers, number_of_stores)
    print(stores_df)
